...
Layout `{ barrelY: 0.46, muzzleZ: 2.30 }`; бюджет ≈ 5.6k вершин (вилка теста 600–20000).

- **Корпус:** приземистая «научная» база: башенное кольцо, нижняя раковина с палубой
  и «подбородком», боковые аппликации, продольные **rail-нити coolant-кондуитов**,
  жалюзи-ловерсы верха, купол с оптикой-глазом, дымовые шашки.
- **Кормовой баллон:** горизонтальная энерго-капсула (`metal`, r 0.34 × 1.46) с
  **5 делениями шкалы** (`rail`) и dark-шиной — визуальный топливный индикатор;
  вместе с нитями и катушками заполняет `rail`-слот ⇒ `railGlowMat` существует
  (инвариант в `turretGeometry.test.ts`).
- **«Выемка вместо ствола»:** ствол-трубы НЕТ. H-образная ярмо-рама (`metal`) с двумя
  рожками-эмитёрами по `x = ±0.30`, на каждом — 3 катушечных кольца (`rail`), седло,
  сужение и **фокусирующий кристалл** (`rail`, z 2.16) + `lamp`-кончик; центральный
  мост с кабель-лотком (dark + lamp-полоса). Мантия — низкая, открытая, с центральной
  dark-**vacuum-выемкой** (0.42×0.34×0.10) и 3 рифлёными планками — «вакуум», из
  которого рожки тянут дугу. Мuzzle — между рожками (`muzzleZ 2.30`).
