...
## Аудио на паузе (hotfix)

`paused` замораживает и весь WebAudio-таймлайн. `GameLoop` каждый кадр
зовёт `audio.setPaused(run.paused)` (`AudioPort`-контракт): `AudioFX`
выполняет `ctx.suspend()` — запланированные голоса (гул заряда рельсы)
замирают посреди огибающей вместо «доигрывания» за затемнением, на снятии
паузы — `ctx.resume()` (авто-resume в `ensure()` гейтится тем же флагом).
Ускоряющиеся тики заряда идут на `setTimeout` и во время паузы пропускаются:
на замороженных часах их ритм бессмыслен, а не пропускать — значит выдать
пачку отложенных блипов при снятии. Идемпотентность держит `AudioFX`
(флаг-зазеркалье), поэтому двойные вызовы безопасны. Вкладка скрыта → rAF
стоит и цикл не успеет вызвать `setPaused` ни в одном кадре —
`GameBootstrap.onVisibility` вешает замок напрямую.
