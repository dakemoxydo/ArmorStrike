...
Слот `flame` — разделяемый на несколько экземпляров оружия, поэтому в
`FlameParticlePool` есть арбитраж (`flameSlot.owner`): пишет только владелец
(первый стреляющий огнемёт держит слот до конца затухания), idle-пул никогда
не трогает риг (иначе гасил чужой свет intensity-0 записями), два активных
не перетягивают слот покадрово.
