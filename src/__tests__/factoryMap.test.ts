 ...
 *
 * The legacy factory module set only covered ~±75 of the 300×300 arena, leaving
 * an empty outer ring (documented as a known gap in Docs/GDD/Approved/Maps.md).
 * The reskin rebuilt it as a full-arena industrial complex. These pins keep the
 * layout from drifting back into the fire lanes, the capture zones or the spawn
 * aprons — the three ways a map edit silently breaks the match.
 */
...
import { buildFactoryContent } from '../game/arena/factoryMap';
...
// Same shape the draw-call census uses: every 2d call is a no-op.
...
describe('factory layout — fire lanes', () => {
  it('keeps the main cross free of hard cover', () => {
    const { colliders } = build();
    const hard = colliders.filter(isHard);
    const inNS = hard.filter((c) => overlapsRect(c, -MAIN_LANE, -150, MAIN_LANE, 150));
    const inEW = hard.filter((c) => overlapsRect(c, -150, -MAIN_LANE, 150, MAIN_LANE));
    expect(inNS.map((c) => [centerX(c), centerZ(c)])).toEqual([]);
    expect(inEW.map((c) => [centerX(c), centerZ(c)])).toEqual([]);
  });

  it('keeps the primary lanes sight-clear end to end', () => {
    const { colliders } = build();
  ...
  it('keeps the centre strip of both primary lanes completely open', () => {
    // A 12 m clear corridor down the middle of each main lane: duels have a
    // real shooting channel, not just "no unbreakable wall".
    const { colliders } = build();
    const strip = 6;
  ...
  it('keeps the centre plaza free of hard cover (CP-B sits there)', () => {
    const { colliders } = build();
...
describe('factory layout — capture points', () => {
  it('leaves every capture anchor reachable and contestable', () => {
    const { colliders } = build();
    const zones = zonesForMap('factory');
    expect(zones).toHaveLength(3);
    for (const z of zones) {
      // room to fight inside the zone
...
describe('factory layout — spawns', () => {
...
describe('factory layout — cover distribution', () => {
  ...
  it('fills the outer band — the legacy empty ring is gone', () => {
    const { colliders } = build();
    const outer = colliders.filter((c) => centerR(c) > 95);
    expect(outer.length).toBeGreaterThanOrEqual(18);
    // content in every cardinal half of the outer band, not just two corners
    const dirs = [
  ...
  it('has enough total cover to be a real arena', () => {
    ...
    expect(colliders.length).toBeGreaterThan(90);
    expect(colliders.filter((c) => c.destructible).length).toBeGreaterThanOrEqual(30);
    expect(blocks.size).toBe(colliders.filter((c) => c.destructible).length);
  });
});

describe('factory layout — ramps and life', () => {
