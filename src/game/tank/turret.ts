...
/**
 * Опорная нано-башня «Изида». Единственная в арсенале башня БЕЗ ствола:
 * вместо дула — открытая Н-образная вилка с двумя рожками-эмитёрами, между
 * ними приёмная воронка вакуума, сзади горизонтальный энергобаллон со
 * светящейся шкалой остатка заряда (rail-слот, его же гонит оружие).
 * Силуэт намеренно «аппаратный», в контраст клиновидной рельсе и кубовой пушке.
 */
function buildIsida(t: TurretBuilder, g: TurretBuilder): TurretLayout {
  turretRing(t, 1.20, 1.26);

  // --- Корпус: приземистая коробка с открытой палубой ---
  t.box(2.10, 0.52, 2.30, 'body', 0, 0.46, -0.10); // lower shell
  t.box(1.94, 0.14, 1.60, 'body', 0, 0.80, -0.30); // deck plate
  t.box(1.90, 0.34, 0.50, 'body', 0, 0.38, 1.10);  // chin nose (рожки выше линии прицела)
  for (const side of [-1, 1] as const) {
    t.box(0.14, 0.40, 2.10, 'metal', side * 1.06, 0.50, -0.10); // side appliqué
    t.box(0.07, 0.07, 2.10, 'dark', side * 1.12, 0.64, -0.10);  // weld seam
    t.rivets(7, 0.035, 'metal', side * 1.10, 0.36, -0.10, 0, 0, 1.90);
    // Криптоновый контур охлаждения: светящаяся нить по борту (rail)
    t.box(0.05, 0.05, 1.70, 'rail', side * 1.15, 0.52, -0.10);
    t.box(0.07, 0.07, 1.74, 'dark', side * 1.13, 0.52, -0.10);
  }

  // --- Задний энергоотсек: горизонтальный баллон со шкалой заряда ---
  t.box(1.70, 0.60, 0.86, 'body', 0, 0.62, -1.32); // bustle base
  t.cyl(0.34, 0.34, 1.46, 14, 'metal', 0, 0.98, -1.32, 0, 0, Math.PI / 2);
  for (const side of [-1, 1] as const) {
    t.cyl(0.35, 0.26, 0.12, 14, 'dark', side * 0.76, 0.98, -1.32, 0, 0, Math.PI / 2);
    t.cyl(0.10, 0.10, 0.08, 8, 'metal', side * 0.84, 0.98, -1.32, 0, 0, Math.PI / 2);
  }
  // Пять сегментов светящейся шкалы (gauge) + шина
  for (let i = 0; i < 5; i++) {
    t.box(0.16, 0.07, 0.05, 'rail', -0.44 + i * 0.22, 1.28, -1.42);
  }
  t.box(1.20, 0.05, 0.08, 'dark', 0, 1.28, -1.40);
  t.rivets(6, 0.035, 'metal', 0, 0.68, -1.78, 1.40, 0, 0);
  // Заборники конденсатного тепла на палубе отсека
  t.louvers(5, 1.40, 0.05, 0.12, 'metal', 0, 1.10, -0.98, Math.PI / 4);

  // --- Командная башенка, прицел-калибратор, антенна ---
  cupola(t, -0.52, 0.88, -0.50, 0.36);
  optic(t, 0.60, 0.80, 0.55, 0.70, 0.10);
  t.cyl(0.05, 0.06, 0.14, 8, 'metal', -0.84, 1.00, -0.86);
  for (const side of [-1, 1] as const) {
    smokeLaunchers(t, side, 0.94, 0.66, 0.40);
    t.box(0.10, 0.10, 0.44, 'metal', side * 1.08, 0.74, -0.68); // grab handle
  }

  // --- Мантия-траверса: низкая, открытая, с приёмной воронкой ---
  t.box(1.30, 0.70, 0.40, 'metal', 0, 0.46, 1.16);
  t.box(1.36, 0.10, 0.44, 'dark', 0, 0.78, 1.16);
  t.box(1.36, 0.10, 0.44, 'dark', 0, 0.14, 1.16);
  for (const side of [-1, 1] as const) {
    t.cyl(0.10, 0.10, 0.44, 8, 'metal', side * 0.30, 0.46, 1.30, Math.PI / 2, 0, 0);
  }
  t.box(0.42, 0.34, 0.10, 'dark', 0, 0.46, 1.34); // вакуумная ниша по центру
  for (let i = 0; i < 3; i++) {
    t.box(0.04, 0.34, 0.06, 'metal', -0.14 + i * 0.14, 0.46, 1.38); // grid bars
  }

  // --- Ствол-вилка: две консоли с рожками-эмитёрами, ствола нет ---
  g.box(0.60, 0.44, 0.40, 'metal', 0, 0, 0.24);  // yoke breech
  g.box(0.86, 0.14, 0.22, 'dark', 0, 0, 0.48);   // crossbar clamp
  for (const side of [-1, 1] as const) {
    const x = side * 0.30;
    g.box(0.16, 0.16, 1.30, 'metal', x, 0, 1.05); // консоль (z 0.40..1.70)
    for (const z of [0.70, 1.05, 1.40]) {
      g.cyl(0.17, 0.17, 0.08, 10, 'rail', x, 0, z, Math.PI / 2, 0, 0); // катушка
      g.cyl(0.13, 0.13, 0.11, 10, 'dark', x, 0, z, Math.PI / 2, 0, 0);
    }
    // Рожок-эмитёр: конусное седло, фокусный кристалл, остриё
    g.cyl(0.12, 0.12, 0.14, 10, 'dark', x, 0, 1.72, Math.PI / 2, 0, 0);
    g.cyl(0.14, 0.09, 0.26, 10, 'metal', x * 0.85, 0, 1.94, Math.PI / 2, 0, 0);
    g.box(0.07, 0.20, 0.07, 'rail', x * 0.72, 0, 2.16);
    g.box(0.05, 0.05, 0.06, 'lamp', x * 0.62, 0, 2.26);
  }
  // Центральная подвеска: мостик между консолями + кабельный жёлоб
  g.box(0.60, 0.05, 0.10, 'metal', 0, 0.14, 1.50);
  g.box(0.10, 0.04, 0.90, 'dark', 0, 0.14, 1.10);
  g.box(0.03, 0.03, 0.86, 'lamp', 0, 0.18, 1.10);

  return { barrelY: 0.46, muzzleZ: 2.30 };
}
