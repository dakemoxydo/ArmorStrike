// ===== МЕНЮ ПАУЗЫ: продолжить, рестарт, гараж, выход в меню =====
import {
...
export default function PauseMenu({ game, muted, stats, onResume, onRestart, onGarage, onMenu, onToggleMute }: PauseMenuProps) {
    ...
    <div className="pointer-events-auto absolute inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative flex w-full max-w-md flex-col items-center px-6">
        ...
        <div className="anim-pop flex flex-col items-center" style={{ '--d': '0s' } as React.CSSProperties}>
          ...
          <h2 className="font-display mt-3 text-4xl tracking-[0.3em] text-white md:text-5xl">
            ПАУЗА
          </h2>
        ...
        <div className="mt-7 flex w-full flex-col gap-2.5">
          <div className="anim-up" style={{ '--d': '0.34s' } as React.CSSProperties}>
            <button onClick={onResume} className="btn-game btn-primary w-full px-8 py-3.5 text-base">
          ...
          <div className="anim-up" style={{ '--d': '0.42s' } as React.CSSProperties}>
            <button onClick={onRestart} className="btn-game btn-ghost w-full px-8 py-3 text-sm">
          ...
          <div className="anim-up grid grid-cols-2 gap-2.5" style={{ '--d': '0.5s' } as React.CSSProperties}>
            <button onClick={onGarage} className="btn-game btn-ghost px-4 py-3 text-[11px]">
              <Wrench size={15} className="bicon" />
              <span>В ГАРАЖ</span>
            </button>
            <button onClick={onMenu} className="btn-game btn-danger px-4 py-3 text-[11px]">
              <ArrowLeft size={15} className="bicon" />
              <span>В МЕНЮ</span>
            </button>
          </div>

          <div className="anim-up flex justify-center pt-1" style={{ '--d': '0.58s' } as React.CSSProperties}>
            <button
              onClick={onToggleMute}
              className="btn-game btn-ghost px-5 py-2 text-[10px]"
