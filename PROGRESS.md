...
## Iteration 19 — 2026-09-11 · [H] HULLS — TITAN (НОВЫЙ СВЕРХТЯЖЁЛЫЙ КОРПУС)

**Task (player request):** «Добавь такой корпус в игру. Это тяжелый медленный
корпус. Назови его Titan. Сделай его таким же красивым и low poly как на фото»
(референс: оливковый low-poly ОБТ над тёмно-серым низом — длинный пологий лоб
со светоблоками, широкие бортовые экраны, круглый вентилятор на моторном отсеке).

**Design-вопрос перед кодом (задан игроку):** «Мамонт» уже занимал нишу
«макс. броня» (160 HP / 11.0). Выбор игрока — **Titan выше Мамонта** (новый
флагман), «Мамонт» остаётся в строю вторым тяжёлым. Ниша «макс. броня» перешла
к Titan, у Мамонта badge → «Тяжёлая броня».

**Scope:** пятая запись каталога — `HullId` расширен до
`'hunter' | 'viking' | 'mammoth' | 'speedy' | 'titan'`. Из этого следует
5 × 3 = 15 loadout'ов вместо 12.

**Implementation:**
- **`catalogTypes.ts` / `catalogData.ts`:** `HullId` + `HULLS.titan`
  (HP 190, speed 9.5, reverse 6.0, turn 1.8) — самая большая прочность и самые
  низкие скорость/разворот в каталоге. Ключ добавлен **последним**: порядок
  существующих ключей не менялся, поэтому раздача корпусов ботам в первых
  четырёх индексах ростера прежняя.
- **`tank/TankConfig.ts`:** `HULL_CONFIG.titan = { type: 'code' }`,
  `HULL_TURRET_Y.titan = 2.36` (кольцо башни на 2.38).
- **`tank/hull.ts` → `buildTitan`:** плита корпуса 3,36 × 4,98; длинный пологий
  лоб `slope(0, 1.76, 2.16, 0.52)` с навесной бронёй, сенсорным блоком с окном
  и **двумя светоблоками по углам** (слот `lamp`); разнесённые бортовые экраны
  с 6 рёбрами; бортовые панели навесной брони с двумя рядами болтов; кормовой
  моторный отсек с **круглой решёткой-вентилятором** (обод + утопленный диск +
  ступица + 8 спиц — единственная такая деталь в наборе) и блоком жалюзи;
  два патрубка с теплозащитой; 8 катков. Ходовой частью задана длина: ленты
  намеренно свисают спереди и сзади за плиту корпуса.
  Габариты **6,01 × 5,00 × 2,68** — самый широкий, длинный и высокий корпус;
  9632 вершины (бюджет 1000–20000).
- **Осознанное исключение из правила §2 «не отдавать крупную площадь под
  `metal`»** (второе после `speedy`): бортовая юбка — крупная *масса* без
  мелкой детали, а двухцветность «тёмный низ / оливковый верх» и есть главная
  черта референса. Рёбра юбки вынесены в отдельные детали слота `dark`.
  Проверено превью в обеих палитрах (player + bot) — при почти чёрном accent
  ботов силуэт читается. Зафиксировано в `Standard_Hull_Models.md` §5.
- **`match/rosterSpawn.ts`:** сват-предохранитель расширен —
  `assault` не берёт `titan` (как и `mammoth`) и уходит в `viking`.
- **`components/HullCard.tsx`:** шкалы брони и скорости больше не хардкодят
  `/160` и `/24`, а считаются от максимума каталога (`MAX_HULL_HP` /
  `MAX_HULL_SPEED`) — новый флагман иначе вылезал за 100 %.
- **`scripts/hull-preview.ts`:** `titan` добавлен в список рендера.

**Tests (контракты обновлены, не ослаблены):**
- `hullGeometry.test.ts` → `keeps the hulls distinct: speedy lowest, titan the
  largest silhouette`: лестница высот `speedy < viking < hunter < mammoth <
  titan`, `titan.x > mammoth.x`, `titan.z > mammoth.z > speedy.z`.
- `botDutyTable.test.ts`: период пары «роль × корпус» вырос с 12 до 15
  (НОК 3 и 5). Золотая таблица переписана на 15 позиций; добавлен пин
  `expect(HULL_IDS).toEqual([...5])`; свапы теперь проверяются на реально
  срабатывающих индексах (i=4 titan→viking, i=7 mammoth→viking, i=6 viking→hunter).
- `botDutyTable.test.ts` (+1 тест): **находка** — при периоде 15 флагман встаёт
  на индексы 9 и 14, а самый длинный ростер — 9 ботов (TDM/CP 4+5, DM 7).
  То есть `titan` ботам **не достаётся ни в одном режиме** — сверхтяжёлый
  корпус сейчас строго игроцкий. Состояние запинено контрактом
  `флагман titan не достаётся ботам ни в одном режиме` (проверяет все индексы
  до потолка ростера): если `teamSize`/`dmBotCount` вырастут, тест упадёт и
  заставит решить осознанно, а не «случайно раздать флагман ботам».
- `Garage.test.tsx` правок не потребовал.

**Verification:** `npm run typecheck` clean; `npm run lint` clean;
`npm test` — **60 files, 345/345**; `npm run build` — ok
(`dist/index.html` 1,166.86 kB / gzip 322.19 kB). Превью:
`npx vite-node scripts/hull-preview.ts railgun player|bot`. Бюджет по слотам —
body 400 / metal 4864 / dark 2840 / track 1456 / lamp 72, 13 draw calls.

**Docs:** `Tank_Movement.md` (строка таблицы корпусов), `Garage_Loadout.md`
(5 × 3 = 15 + новая секция «Лестница корпусов»), `00_Index.md` (5×3),
`Standard_Hull_Models.md` (бюджет, габариты, дизайн, лестница силуэтов,
исключение по `metal` стало таблицей из двух корпусов). Черновик
`Drafts/Hull_Titan.md` заведён в Phase A и удалён после синхронизации Approved.
