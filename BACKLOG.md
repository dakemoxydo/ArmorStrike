...
- [x] J7: Разобрать `npm audit` (10 уязвимостей в предсуществующих deps). — 2026-09-11: **10 → 2**. `npm audit fix` (не-breaking) закрыл 7: `brace-expansion` (high), `browserslist` (high), `js-yaml` (high), `nanoid` (high), `baseline-browser-mapping`, `postcss` (moderate) + `esbuild`. `vite` поднят `7.3.2 → 7.3.6` (патч, обе high — Windows-only dev-server: NTLMv2 hash disclosure через UNC и `server.fs.deny` bypass; актуальны именно для этой машины). `esbuild` запинен `overrides: 0.28.2` — vite тянет `0.27.7`, а дыра `GHSA-g7r4-m6w7-qqqr` закрыта в `0.28.2`, что внутри разрешённого vite диапазона `^0.27.0 || ^0.28.0`. Гейт после обновления зелёный, subfolder-гейт PASSED. **Остаток — 2 moderate от `vitest`/`@vitest/mocker` (GHSA-82fw-gwwq-j7x9, CVE-2026-84373), принят осознанно.** Разбор: (1) патч существует — `4.1.11` (проверено по первоисточнику advisory; npm-audit предлагает только `5.0.0`, т.к. берёт наивысший патч), и `4.1.11` сохраняет Node 20, в отличие от `5.0.0` (`^22.12 || ^24 || >=26` против заявленных в проекте `^20.19.0 || >=22.12.0`); (2) **но npm-клиент в этой среде не может разрешить `vitest@4`** — arborist падает `TypeError: Cannot read properties of null (reading 'edgesOut')` в `#loadPeerSet` при построении `node_modules/vitest` (npm 10.9.7, воспроизводится и без `overrides`; обход через `--legacy-peer-deps` требует `.npmrc` и меняет семантику `npm ci` в CI — хуже, чем сам риск); (3) экспозиция проекта практически нулевая: advisory эксплуатируется через HMR-WebSocket dev-сервера, а неаутентифицируемый путь — только через публичные `mockerPlugin`/`interceptorPlugin` для сторонних dev-серверов; проект гоняет headless `vitest run` (node + точечно jsdom), browser mode и dev-сервер vitest не использует, плагин наружу не экспортирует. Итог: оставлено на `3.2.7`; апгрейд до `4.1.11` — отдельная задача вместе с чистым `node_modules`.
