...
- [ ] B8 (**MED**): City без инстансинга: 705 рисуемых геометрий / 491 дубликат (36 фонарей и 4 светофора со своей геометрией, 18+ jersey, 12+ плантеров, окна офисов на панель — `src/game/arena/cityMap.ts`); Паттерн есть в village (fence/trees: 34 InstancedMesh). Частично 2026-09-15: `shell.ts` — 20 пилонов на общем материале (clone() убран) + 1 геометрия ламп на 14 фонарей. Ядро (перевод city-декора на InstancedMesh) — отдельная рискованная перестройка билдера, не начиналась.
...
## [G] AUDIO
- [ ] G1: Verify spatialization hooks for enemy fire (relative-position panning) exist and are wired for cannon/railgun/flamer. — аудит: теперь 5 оружий, также проверять gauss/isida.
...
- [ ] J11 (low): Аллокации против fillAmmoState-паттерна: per-frame — `getAmmoState()` без sink (`src/game/engine/stages/PlayerInputStage.ts:22`), cone-литерал в IDLE (`src/game/weapons/GaussWeapon.ts:169-176`, изида переиспользует модульный), массив origins каждый кадр луча (`src/game/weapons/NanoBeamFx.ts:125`), clone() на выстрел (`src/game/weapons/GaussBeamFx.ts:55`); per-shot у рельсы — tmpMuzzle/tmpDir/hit.point clones (:267-268,435), new Set (:408).
