...
- [x] B5: Устранить фризы/лаги на выстрелах и смертях. — 2026-09-12: root cause — динамические `scene.add/remove(PointLight)` меняли число источников света, а оно входит в ключ кэша шейдерных программ (`numPointLights`) → пересборка ключа у всех lit-материалов + синхронный GLSL-компил в кадре. Фиксы: (A) `effects/LightRig` — постоянный бюджет 7 источников (flash 4 / beam 2 / flame 1), эффекты только пишут `intensity`, никогда не attach/detach; (B) `RenderWorld.warmUp()` (`compileAsync` под loading-overlay) в `executeStartRound`; (C) hit-stop/slow-mo гейтится `byPlayer` — бот-vs-бот больше не морозит матч; (D) `WreckSystem` переписан на пул 6 слотов без аллокаций и без `material.dispose()` на смерть; (E) HUD — убран принудительный reflow в `shotFired`, nameplate квантован (`HP_DRAW_STEP = 0.02`). Гейт: 62 файла / 350 тестов ✓, typecheck ✓, lint ✓. Док — `Docs/Architecture/Standard_Frame_Stability.md` + GDD `Kill_Feedback.md`
